...
fs.readdir(USER_DATA_PATH, (err, files) => {
  ...
  if (files.length <= 1) {
    fs.readFile(USER_DATA_PATH + files[files.length - 1], (err, data) => {
      ...
      parsedData.id = 1;
      fs.writeFileSync(
        USER_DATA_PATH + files[files.length - 1],
    ...
    fs.readFile(USER_DATA_PATH + orderedContent[0].file, (err, data) => {
      ...
      Object.keys(parsedData).indexOf("id") === -1
        ? (parsedData.id = orderedContent.length)
        : null;
      fs.writeFileSync(
        USER_DATA_PATH + orderedContent[0].file,
        JSON.stringify(parsedData)
      );
      ...
      files.forEach((fileName, index) => {
        ...
        let parsedData = JSON.parse(data.toLocaleString());
        allUserData[parsedData.id  || index] = parsedData;
      });
...
const getOrderedFilesInDirectory = () =>
  fs
    .readdirSync(USER_DATA_PATH)
